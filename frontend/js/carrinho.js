...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', () => {
          // Por enquanto, apenas leva para a seleção de endereço
    ...
    if (checkoutBtn) {
      checkoutBtn.addEventListener('click', () => {
        alert('Fluxo de finalização ainda será implementado.');
        // Futuro: redirecionar para tela de endereço/pagamento
      });
    }
  });
  ...
  function renderCart() {
    ...
    itemsContainer.innerHTML = items
      .map((item) => {
        ...
        const precoFormatado = (item.preco || 0)
          .toFixed(2)
          .replace('.', ',');
          ...
          <div class="d-flex align-items-center gap-3 border-bottom py-3 cart-item" data-id="${item.id}">
            ...
            <div class="flex-grow-1">
              <div class="d-flex justify-content-between">
                ...
                <button
                  type="button"
                  class="btn btn-link btn-sm text-danger p-0 cart-remove-btn"
                >
              ...
              <div class="d-flex justify-content-between align-items-center mt-2">
                <div class="d-flex align-items-center gap-2">
                  <button
                    type="button"
                    class="btn btn-outline-secondary btn-sm cart-qty-decrease"
                  >
                    -
                  </button>
                  <input
                  ...
                  <button
                    type="button"
                    class="btn btn-outline-secondary btn-sm cart-qty-increase"
                  >
                    +
                  </button>
                </div>
    ...
    totalSpan.textContent = `R$ ${subtotalFormatado}`; // por enquanto, total = subtotal
